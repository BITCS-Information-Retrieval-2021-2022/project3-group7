...
项目基于`` Django``及`` Bootstrap`` 搭建，其中使用``Django``实现页面之间的跳转及页面间参数的传递

(1)整体逻辑

进入search界面，输入关键字并选择是否使用引文网络重要性分数改善；检索结果页面展示检索服务返回的论文列表，点击论文标题可进入子图绘制页面
 ...
 a.search.html ——  查询页面
 b.display.html —— 论文列表展示页，返回匹配的论文列表，并根据用户选择决定是否通过重要性分数对论文进行排序；
 c.graph.html —— 引文网络展示页，通过向检索服务发送给定的论文标题/Sid，获取论文子图的json表示，进而绘制论文引用子图；

(3)运行说明

运行项目，随后访问127.0.0.1或127.0.0.1/search便可进入主页面
